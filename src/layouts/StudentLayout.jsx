...
const StudentLayout = ({ children }) => {
  ...
  // Reset sidebar state when exiting lesson mode
  useEffect(() => {
    if (!isLessonMode) {
      setSidebarOpen(true);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Main content */}
      <div className={`${!isLessonMode ? 'lg:pl-64' : ''} transition-all duration-300`}>
        {/* Top bar: hidden in lesson mode */}
        {!isLessonMode && (
        ...
        {/* Page content */}
        <main className={`${!isLessonMode ? 'p-6' : 'p-0'}`}>
          {children}
        </main>
      </div>

      {/* Overlay for mobile */}
      {!isLessonMode && sidebarOpen && (
        <div
